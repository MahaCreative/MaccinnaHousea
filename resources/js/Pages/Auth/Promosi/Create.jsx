...
export default function Create({ model, setModel, setOpen }) {
    ...
    const updateHandler = (e) => {
        ...
        ResponseMethode(
            ...
            () => {
                post(
                    route("auth.update-promosi", model.id),

                    {
                        onSuccess: () => {
                            showResponse(
                                "success",
                                "Sukses",
                                "Berhasil memperbaharui data promosi"
                            );
                            setModel();
                            reset("gambar", "kontent", "title");
                            setPreviewImage("");
    ...
    const submitHandler = (e) => {
        ...
        post(route("auth.store-promosi"), {
            ...
            onSuccess: () => {
                ...
                showResponse(
                    "success",
                    "Sukses Tambah Promosi",
                    "1 Foto baru berhasil di tambahkan kedalam promosi"
                );
            },
            onError: (err) => {
                showResponse(
                    "error",
                    "Gagal",
                    "gagal menambahkan 1 promosi baru, silahkan periksa kembali isian anda"
                );
            },
        });
    };
    ...
    return (
        ...
        >
            ...
            >
                <div className="absolute top-2 left-2 bg-blue-400 text-white py-1 px-2 font-thin rounded-md">
                    Change Picture
                </div>
                <img
                    src={`${
            ...
            <div className="flex justify-between items-center">
                <h2 className="text-blue-400 font-light">Deskripsi Foto</h2>
                <div className=" flex gap-3 items-center">
                    <button
                        type=""
                        className="bg-primary hover:bg-blue-600 useTransition py-1.5 px-4 text-white font-light rounded-md"
                    >
                        Submit
                    </button>
                    <button
...
Create.layout = (page) => (
    <AuthLayout children={page} title={"Create promosi"} />
);
